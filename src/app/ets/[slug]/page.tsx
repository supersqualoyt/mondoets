...
export default async function EtsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <section className="mt-12 p-6 bg-brand-50 rounded-lg border border-brand-100">
        <h2 className="text-xl font-bold mb-2">Sei {ets.denominazione}?</h2>
        <p className="text-gray-700 mb-4">
          Rivendica la tua scheda per aggiornare i dati pubblici e accedere a strumenti dedicati ai soci di Sesto Potere APS.
        </p>
        <Link
          href={`/diventa-socio?ets=${encodeURIComponent(ets.codiceFiscale)}`}
          className="inline-block px-5 py-2 bg-brand-600 text-white font-semibold rounded-md hover:bg-brand-700 no-underline"
        >
          Rivendica scheda
        </Link>
      </section>
